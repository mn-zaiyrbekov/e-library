...
export const actions = {
  async setBooksForUser({commit}, {idUser,idBook}){
    try{
      const res = await this.$axios.post(`/books/books/user/set`, { idUser: idUser, bookId: idBook })
      return res
    }catch(err) {
  ...
  async getUserBooks({commit}, idUser) {
    try {
      const res = await this.$axios.get(`/books/books/user/${idUser}`)
      if(res.data.success) {
  ...
  async deleteUserBooks({commit}, { idUser, bookID }) {
    try {
      const res = await this.$axios.put('/books/books/user/delete', {
        userId: idUser,
        bookId: bookID
      })
      if (res.data.success) {
