...
export const actions = {
  async setBooksForUser({commit}, {idUser,idBook}){
    try{
      const res = await this.$axios.post(`/books/user`, { idUser: idUser, bookId: idBook })
      return res
    }catch(err) {
  ...
  async getUserBooks({commit}, idUser) {
    try {
      const res = await this.$axios.get(`/books/user/${idUser}`)
      if(res.data.success) {
  ...
  async deleteUserBooks({commit}, { idUser, bookID }) {
    try {
      const res = await this.$axios.put('/books/', {
        userId: idUser,
        bookId: bookID
      })
      if (res.data.success) {
